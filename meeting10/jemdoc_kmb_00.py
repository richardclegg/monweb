...
def standardconf():
  ...
  [lastupdated]
  Page generated |, by <a href="http://jemdoc.jaboc.net/">jemdoc_kmb</a>.
...
def semicolonlist(f,ordered=False): 
  # Keith Briggs 2011-09-13 15:39
  # no nesting
  def surname(a,b): # Ughh - assumes surname is second :(
    return cmp(a.split()[-2],b.split()[-2])
  if ordered:
    char=';'
    ul='ol'
  else:
  ...
  while pc(f)==char:
    (s,newlevel)=np(f,True,False)
    items.append(br(s,f).lstrip(';'))
  items.sort(cmp=surname)
  out(f.outf,'<%s>\n'%ul)
  for item in items:
    out(f.outf,'<li><p>'+item+'</p></li>')
  out(f.outf,'</%s>\n'%ul)
