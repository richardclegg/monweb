...
"""jemdoc version 0.6.6, 2009-07-11."""

# Copyright (C) 2007-2009 Jacob Mattingley (jacobm@stanford.edu).
#
...
def standardconf():
    ...
    [lastupdated]
    Page generated |, by <a href="http://jemdoc.jaboc.net/">jemdoc</a> -- no cookies are used on this webpage.
...
def gethl(lang):
    ...
    if lang in ('py', 'python'):
        ...
        d['special'] = putbsbs(['cols', 'optvar', 'param', 'problem', 'norm2',
                                ...
                                'rows', 'rand', 'randn', 'printval', 'matrix'])
        d['error'] = putbsbs(['\w*Error',])
        d['commentuntilend'] = '#'
        d['strings'] = True
    elif lang == 'c':
        d['statement'] = putbsbs(['if', 'else', 'printf', 'return', 'for'])
        d['builtin'] = putbsbs(['static', 'typedef', 'int', 'float', 'double',
                                'void', 'clock_t', 'struct', 'long'])
        d['special'] = (['#include.*', '#define', '@pyval{', '}@', '@pyif{',
                         '@py{'])
        d['error'] = putbsbs(['\w*Error',])
        d['commentuntilend'] = ['//', '/*', ' * ', ' */']
        d['strings'] = True
    elif lang == 'sh':
        d['statement'] = putbsbs(['cd', 'ls', 'sudo', 'cat', 'alias',
                                  'for', 'do', 'done', 'in',
                                  ])
        d['operator'] = ['&gt;', r'\\', r'\|', ';', '2&gt;', 'monolith&gt;']
        d['builtin'] = putbsbs(['gcc', 'python', 'curl', 'wget', 'ssh',
        ...
        d['statement'] = putbsbs(['max', 'min', 'find', 'rand', 'cumsum'])
        d['operator'] = ['&gt;']
        d['builtin'] = ['ans =', '>>']
        d['commentuntilend'] = '%'
        d['strings'] = True
...
def language(f, l, hl):
    # class is capitalized to avoid matching the class keyword.
